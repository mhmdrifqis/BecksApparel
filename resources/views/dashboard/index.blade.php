...
<body class="bg-navy-950 font-sans antialiased text-slate-300" x-data="{ sidebarOpen: false, userDropdownOpen: false }">
                            ...
                            <ul class="py-1" role="none">
                                <li>
                                    <form method="POST" action="{{ route('logout') }}">
                                        @csrf
                                        <button type="submit" class="block w-full text-left px-4 py-2 text-sm text-red-400 hover:bg-navy-800 hover:text-red-300 font-bold" role="menuitem">Keluar</button>
                                    </form>
                                </li>
                            </ul>
                        </div>
    ...
    <aside id="logo-sidebar" 
        ...
        <div class="h-full px-3 pb-4 overflow-y-auto bg-navy-900 custom-scrollbar">
            <ul class="space-y-2 font-medium">
                ...
                @if(auth()->user()->isManajemen())
                    <li class="pt-4 mt-4 space-y-2 border-t border-slate-800">
                        <span class="px-2 text-xs font-bold text-slate-500 uppercase tracking-wider">Laporan Eksekutif</span>
                    </li>
                    <li>
                ...
                {{-- @if(auth()->user()->isPelanggan())
                    <li class="pt-4 mt-4 space-y-2 border-t border-slate-800">
                        <span class="px-2 text-xs font-bold text-slate-500 uppercase tracking-wider">Belanja</span>
                    </li>
                    <li>
                        <a href="#" class="flex items-center p-2 text-slate-300 rounded-lg hover:bg-navy-800 hover:text-lime-400 group">
    ...
    <div class="p-4 sm:ml-64 mt-14 bg-navy-950 min-h-screen">
